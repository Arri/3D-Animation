#############################################################
# 3D engine for animation with TKinter
#
...
class Engine3D:
    def __init__(self, points, shapes, width=1000, height=700, distance=6, size=100, title='3D', background='white'):
        #object parameters
    ...
    def __keypress(self, event):
        ...
        if event.keysym == 'Up':
            ...
            self.size += 1
        elif event.keysym == 'comma' and self.size > 1:
            self.size -= 1
        self.clear()
        self.render()
    ...
    def writeShapes(self, shapes):
        ...
        for shape in shapes:
            if type(shape[-1]) != str:
                shape.append(faceColor)
            self.shapes.append(graphics.face.Face(shape))

    def changeCoordinates(self, x, y, z):
        self.rotate('x', x)
        self.rotate('y', y)
        self.rotate('z', z)
    ...
    def render(self):
        ...
        points = []
        for point in self.points:
            points.append(point.flatten(self.size, self.distance))
